...
class Component extends View {
    ...
    try {
      if (!this.el.dataset.mounted) {
        this.el.insertBefore(
          jst.compileToDOM(this.template),
          this.el.children[0]
        );
      }
      return this;
  ...
  initModel() {
    this.model =
      this.options.model || new this.modelClass(this.options.modelData);
    return this;
  }
  ...
  initCollection() {
    this.collection =
      ...
      new this.collectionClass(this.options.collectionData);
    return this;
  }
  ...
  ensureElement(el) {
    this.el = !el
      ? document.createElement(Component.defaults.el)
      : isElement(el)
      ? el
        ...
        [].forEach.call(components, componentEl => {
          ...
          if (Resolver.has(componentId)) {
            ...
            throw new ReferenceError(
              componentId + " not found in component resolver.",
              Resolver
            );
          }
        });
