...
class Collection {
  ...
  initProps(data, options) {
    this.model = options.modelClass
      ...
      : options.model || new this.modelClass(data);
    this.models = [];
    this.parse(data);
  }
  ...
  parse(data) {
    ...
    try {
      if (!data && !data.length)
        throw new Error("Incoming data has zero length");
      this.add(data);
    } catch (e) {
      console.warn(e);
    }
  ...
  fetch(options = {}) {
    return this.request(this.verifyResource(options.url), options).then(
      response => {
        this.parse(response.data);
      },
      this.onParseFailed.bind(this),
      "collection.fetch"
    );
  }
